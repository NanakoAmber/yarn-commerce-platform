...
> 当前命名决定：2026-09-13 用户在核对最新 GitHub / Shopify 基线后确认正式名称为小写 `mewool`（Issue #49）。保留本文猫抱毛线球图形与字形风格，只缩短字标。以下记录旧版资产的真实来源与发布事实，不表示短版已接入或上线；现行品牌约束以 PRODUCT.md 的 Brand Commitments 为准。
