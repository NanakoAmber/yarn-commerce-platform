...
- 当前 live Theme `189727637817` 打开团队演示模式：所有具有标题的 Project 都会展示，缺少封面不会阻断进入详情。
- Metaobject 的 Active 只是 Shopify 平台展示状态；团队演示可见不代表材料、教程、版权、语言或商品已经达到公开标准。
- 定制咨询发送保持关闭。用户取消本轮测试邮件，实际收件未验证，不发送测试咨询。
...
2. 填「规格与用途」，选「供给方式」：本店提供、需自行准备、信息待补充；再把「配件类型」标为毛线、工具或其他材料。不要把配件未备齐当作隐藏作品的原因。
3. 本店提供的项目通过「选择多属性」关联**具体商品规格**；价格和库存仍在原商品维护。没有有效关联不会显示购买按钮；缺货自动显示暂时缺货，不用手工维护第二份库存。
4. 每个作品至少为一条「毛线」配件选择「配件图片」。优先使用已确认的商品图；没有图时可在受保护内部原型使用与作品配色一致的生成示意图，并在「补充说明」明确披露。
5. 可填「建议采购数量」和「补充说明」。采购件数不是作品耗用量；内部演示的数量与适配必须标注待核实。顾客可自行改购买数量，按项加入，工具不自动加入。
6. 回到编织作品，打开「逐项配件清单」，按希望展示的顺序选择条目并保存。选择器的「添加新条目」是就地新建入口；已有条目可从配件列表修改，引用作品会同步读取。
7. 到指定未发布 Theme 预览刷新，先确认毛线以大图与标题出现，再核对名称、说明、供给状态、商品规格与顺序。非本店供给项没有购买按钮，其他项仍可选购；从教程可返回同一清单。
