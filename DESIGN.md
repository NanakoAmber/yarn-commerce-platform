...
Issue #39（2026-09-09）的首页内容区与独立搜索继承此视觉世界，当前构图以[搜索与首页 surface brief](docs/design/search-surface.md)及其中两张获批稿为准。下文当前首页 / 搜索规则覆盖旧版原地搜索、旧卡片信息密度与分类图尺寸；Hero、共享导航、帮助、页脚和详情既有方向继续保留。此次提取依据 `yarn-discovery.css`、共享 tokens、Section / card snippets，并打开复核了未发布 Theme `189819093305` 在 CSS 1440 / 390 px、1x 浏览器下的中 / 日 / 英桌面与手机共 12 张成品截图。该范围只确认所见构图，不代表账号收藏跨设备、后台编辑、全目录翻译、全部 gates 或发布验收完成；完成状态以 Issue / PR 为准。
...
- 当前首页以连线和波浪串联，作品精选和分类行平铺，成品区、帮助与页脚使用浅奶油纸面，导航使用陶土动作色；叠压流程纸面属于保留的历史 B 组件。
...
- **选中暖墨**（`selected-ink`）：历史 B 发现类型与搜索光标；当前首页、独立搜索和共享导航 / 页脚输入光标使用行动陶土色。
...
Issue #39 的能力标签复用浅奶油、`capability-blush` 与 `capability-mint`，分别承接有成品 / 双能力、仅材料与仅灵感；使用深色文字，不把柔色当作库存或齐套证明。价格继续使用炭黑，不新增价格强调色。
...
- **Headline**：`headline-discovery` 保留历史 B 的居中发现标题。共享 Section 标题通过 `shared-section` / `--yarn-type-section` 表达。Issue #39 首页搜索引导标题局部为 clamp(2.6rem, 3.2vw, 4.2rem)，行标题桌面 2.6rem、手机 2.2rem；搜索结果组标题桌面 2.6rem、手机 2rem。页面层级不重定义共享 Section 令牌。
- **Title**：`title-work` 保留历史 Product 卡角色；Issue #39 首页与搜索采用 `discovery-card-title` 及手机覆盖，最多两行；不再沿用旧首页 1.65rem / 500 的名称样式。
...
Theme 的 `--font-body-scale` 为 1.0，但源码 `rem` 仍取决于宿主根字号。较早一轮用户 Chrome 实测 `document.documentElement` 根字号为 12.5 px；这是历史预览环境证据，不是 Issue #39 的当前事实或全站默认。新增文案仍须检查三语断行，不能把源码 rem 在未知根字号的环境中直接当作固定 px。

Issue #39 的首页与搜索共用前言 `discovery-card-title` / `discovery-price` 及手机覆盖。简介桌面 1.5rem、手机 1.3rem；首页为对齐标签 / 价格保留两行标题高度，简介桌面一行、手机最多两行，搜索简介最多两行。首页标题行高局部为 1.6，不替代搜索的 1.5；截断不改变原始链接名称和内容来源。
...
Issue #39 首页分类入口保持四项一行，桌面位于搜索右侧，749 px 以下按搜索、能力筛选、分类顺读。作品精选与精选成品共用四列上图下文网格；749 px 以下均为两列，列距由 24 px 改为 16 px。精选成品以单块浅奶油外层区分，卡片自身透明；布局保留四槽，不以当前商品数改为三列。后续分类预览轨道沿用桌面四槽、中间宽度三槽、手机两槽与溢出箭头，查看全部进入对应搜索浏览。短行按真实内容留白，不复制作品或商品填槽。

独立搜索共用 1320 px 内容上限与响应式 gutter，搜索栏和分类条最大宽度 720 px。作品结果桌面为两列横向图文卡，989 px 以下成为单列；商品桌面四列，750–989 px 三列，749 px 以下两列。手机作品图宽 114 px、图文间距 16 px；图片比例 1.04，商品手机比例 1.2。结果组用留白与细线分开，没有命中的组不保留空容器。这些构图只用于当前搜索 surface。

手机保住主说明、主要操作与四节点标签净空；对象插画完整显示，不得覆盖正文。装饰定位和当前标题断行留在 surface brief 与代码，不作为其他页面的绝对定位模板。

首页 Project 库当前采用 Issue #39 精选网格与分类预览合同；作品和成品媒体宽高比同为 1.29，14 px 圆角，封面通过 cover 填充，不宣称每张图片完整展示。以下两段保留 Issue #23 / #28 的历史详情布局，已由 Issue #34 替代：详情桌面首段为约 1.05:1 的作品图 / 文字双栏，之后回到最大 760 px 的单列阅读流。749 px 以下的 `paper_flow` 首段先以 112 px 作品图与完整标题并排，列间距 14 px，图像 `contain` 保住完整作品；简介、事实和演示披露随后各自跨满两列，后续保持单列顺读。事实按最小 84 px 自适应分列，以 8 px 间距和上下留白收紧排列，披露只合并换行、不隐藏内容。三语 390×844 首屏中，完整标题、简介、必要事实、演示披露以及“制作准备”标题与首条有意义的信息均在 sticky CTA 上方可见。这个首屏合同只属于作品详情，不推广成其他页面的高度公式。
...
当前首页作品行平铺且无外框阴影，帮助与页脚以浅奶油面及细边线区分，串联 Hero 不使用叠压纸卡阴影。历史 B 仍保留柔纸叠层：流程纸面使用轻扩散阴影，自定义筛选展开面板因覆盖结果区而使用更清楚的扩散阴影；这些不是当前独立搜索筛选的默认深度；Issue #39 搜索胶囊与结果卡不增加阴影，筛选 dialog 通过暖白面与共享半透明遮罩区分覆盖层。历史 Project 主毛线纸面不用新增阴影，只靠暖白表面、14 px 局部圆角、细边线和内距建立一层焦点；辅助清单回到无卡片的分隔线。共享焦点外圈是交互状态，不是卡片深度。
...
当前首页以胶囊搜索框、柔圆角作品照片与自然插画轮廓组织形状。四分类沿用既有批准原画，图像以 contain 完整容纳；Issue #39 首页与搜索桌面图位均限制在 112 px 以内，首页手机上限 96 px，搜索手机为 44 px。这些 CSS 限制没有提高源图分辨率，现有低清和方底问题不作为材质规范，也不授权重新生成。历史 B 小圆针目 plate 不再是当前首页图标。帮助条保持中等柔圆角，保留的套件标签仍为小圆角粉色标签，不是按钮。

共享 Rise 购买控件保留小圆角，旧暖黄动作保留胶囊及较宽圆角形态；这些既有形状没有被全站禁止。历史 B 筛选为圆角矩形触发器；旧 Project 库高级筛选使用原生 details / select；Issue #39 独立搜索改为简单能力 radio 与 dialog，不沿用旧复杂筛选合同。
...
Issue #39 的作品图片与文字仍组成 Project 详情链接，不重复逐卡“查看作品”CTA；各行“查看全部”链接至独立搜索的对应浏览，成品卡链接至 Product 详情。原行“查看全部 / 收起”属于旧版行为。双箭头放在该行图片与名称下方、右对齐，44×44 px 圆形触控区，陶土描边与动作色；只有溢出时出现，端点禁用。禁用态采用 token 源码中的 #8c8880 文字与 #d3cfc7 描边，不提升为推荐内容色；hover 复用 `action-hover`。轨道支持原生横向滚动，减少动态偏好下按钮滚动不使用平滑动画。历史 B 的查看材料、类型按钮组和对应选中线保留在旧组件中。
...
Issue #39 首页能力筛选沿用可换行胶囊和 44 px 最小触控高度，选中态为奶油黄底与炭黑文字，未选为暖纸底；替代 Issue #34 的陶土描边选中态。作品卡可同时显示成品 / 材料能力，两种能力均无显示灵感；标签桌面 1.3rem、手机 1.2rem，不代表库存或齐套。商品卡以炭黑价格承接末行，不附作品能力标签。

### Cards / Containers

Issue #39 首页作品卡以 Shopify `yarn_project.cover`、名称、用途简介与同源能力标签为主体，成品卡沿用相同图片、标题、简介和爱心位置，以真实价格替代能力标签。媒体宽高比为 1.29、cover 填充；不在卡片添加未经验证的难度、时间或社交数量。Hover 为名称下划线，无外卡框、阴影或抬升。历史 B Product 卡的 0.93 媒体比例、contain 与“查看材料”仍留在旧组件，不能作为当前分类行的卡片合同。
...
Issue #39 首页搜索为暖白胶囊、暖灰细边和至少 56 px 高，搜索图标位于左侧；提交进入统一搜索页，不再在首页原地搜索全库。独立搜索在 990 px 起使用与结果区同宽的横向工具栏：左侧输入、右侧常驻购买条件，空间不足时整组换行；输入最小高 64 px、共享 24 px 圆角，清空 X 保留在字段内，桌面隐藏重复范围说明与弹窗触发器。989 px 以下沿用胶囊、范围说明和独立滑杆按钮，749 px 以下最小高 64 px；这些尺寸属于搜索 surface，不覆盖共享页脚字段。

分类条使用手绘图像与 HTML 标签，当前选中项以陶土下划线区分。990 px 起五类同宽横排、图文左右组合；标题、工具栏、分类、默认示例词和结果使用同一内容边界。简单能力筛选使用带 label 的原生 radio：全部、有成品、可买材料、仅灵感；桌面常驻并即时生效，中等宽度使用居中 dialog，手机使用底部纸面。两种控件共享条件、浏览器返回及键盘单选行为，切至桌面会关闭仍打开的面板并将焦点交给已选条件。空结果、载入失败和重试保留独立可读状态，不把缺失结果解释为完整目录已查无内容。

### Favorites

首页与搜索作品 / 商品卡共用图像右上角的暖白圆形爱心按钮（`button-favorite`），44 px 触控区、22 px SVG；选中及 hover 使用行动陶土色与浅奶油底，键盘焦点沿用共享灰蓝轮廓。作品横行的爱心仍定位在图像上，不占正文列；收藏入口位于搜索标题栏右侧及共享导航。爱心表达收藏，不显示人数、评分或点赞数。视觉选中态不等同账号保存成功，跨设备找回仍须独立验证。
...
- **Don't** 将当前分类原画的低清方底、运营译文缺失、示例商品数量或收藏服务未开放状态写成设计规范；源码与截图中的局部实现不替代获批方向和实际验收。
