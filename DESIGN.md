...
Issue #28 的首页批注子范围仅收紧页眉与首屏：品牌收短为 `mewool`；访客主导航固定为“首页 / 成品 / 原材料 / 联系方式”并提供日英对应文案；主张改为“喜欢的作品，不该卡在选材料。”并明确毛线、用量、工具与教程的准备价值；首屏改用同时出现成品花束、毛线、钩针与织物的真实编辑照片，移除 discovery 首屏的格纸和针目涂鸦。这个子范围保留已批准的作品优先 B 构图、三步路径、发现区、商品事实边界和购买流程，不构成全站重新设计。
...
系统保留柔纸叠层，但 B 降低了连续大卡的存在感：流程纸面使用轻扩散阴影，作品卡平铺且无外框阴影，帮助条以浅蓝面区分。筛选展开面板因覆盖结果区而使用更清楚的扩散阴影。Project 主毛线纸面不用新增阴影，只靠暖白表面、14 px 局部圆角、细边线和内距建立一层焦点；辅助清单回到无卡片的分隔线。共享焦点外圈是交互状态，不是卡片深度。
...
本次串联首屏为静态插画和 SVG 连线，已移除旧三步入场与箭头动画。作品库类别 hover 的位移在减少动态偏好下关闭；其他页面的历史动画不因本次复核而取得全站认证。
