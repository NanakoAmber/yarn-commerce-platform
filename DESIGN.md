...
Issue #39 首页搜索为暖白胶囊、暖灰细边和至少 56 px 高，搜索图标位于左侧；提交进入统一搜索页，不再在首页原地搜索全库。独立搜索胶囊同时承载关键词和「作品 · 商品」范围说明，清空 X 位于胶囊内，滑杆筛选按钮独立在右侧，二者使用不同 SVG。桌面胶囊最小高 74 px，749 px 以下为 64 px；这些尺寸属于搜索 surface，不覆盖共享页脚字段。

分类条使用手绘图像与 HTML 标签，当前选中项以陶土下划线区分。简单能力筛选使用带 label 的原生 radio：全部、有成品、可买材料、仅灵感；类别不在面板重复。筛选面板桌面为居中 dialog，手机为底部纸面与上方柔圆角。空结果、载入失败和重试保留独立可读状态，不把缺失结果解释为完整目录已查无内容。
