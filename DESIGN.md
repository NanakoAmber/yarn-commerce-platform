...
Issue #23 在这个既有世界内新增 Project 库与详情：列表继续以真实织物为入口，详情采用用户已批准的 `paper_flow` 移动首屏，并把作品事实、制作准备、同页 Tutorial 与购买分流放在顺读纸面中。独立 finish review 的两项修复——390 px 首屏露出制作准备，以及移动购物车数量控件完整——在最终复拍中均为 `resolved`，verdict 为 `ship`；此结论只覆盖这两项，不代表英文发布、咨询发送、旧首页 build gate 或整体技术验收通过。
...
- Project 详情以真实作品图、紧凑事实和可见制作准备兑现移动首屏，底部制作入口持续可达。
...
- **Project Title**：详情桌面使用响应式展示标题；749 px 以下固定为 2.4rem、1.3 行高，完整显示真实 Project 名称，不以截断换取首屏空间。2.4rem 仅在根字号 10 px 时等于 24 px；本轮用户 Chrome 实测根字号为 12.5 px，因此当前呈现为 30 px，不能把任一浏览器换算值写成全站固定像素规范。
...
Project 库沿用 1160 px 内容壳：宽屏四列、989 px 以下两列；卡片使用 1:1 真实作品图。详情桌面首段为约 1.05:1 的作品图 / 文字双栏，之后回到最大 760 px 的单列阅读流。749 px 以下转为单列 `paper_flow`：封面高度收敛在 `clamp(140px, 19svh, 180px)` 并以 `contain` 保住完整作品，事实使用可自适应的三列布局；标题、summary、三项事实、完整“内部演示”披露以及“制作准备”标题与首条信息必须在 sticky CTA 上方可见。这个首屏合同只属于作品详情，不推广成其他页面的高度公式。
...
语言菜单继续使用已有本地化链接和 Shopify 的可用语言集合。较早一轮记录显示当时菜单列出日文 / 简体中文且 `/en` 可直接访问并渲染英文；这不是 Issue #23 的当前验证结果。Issue #23 英文仍未发布、未验证，不能将历史 URL 可访问记录写成当前英文路径或所有市场语言菜单均已通过。

### Project Library / Detail

Project 卡片与 Product 卡片保持不同语义：1:1 真实作品图下方只呈现 Project 名称和类别 / 难度等事实，整卡链接进入统一详情，不链接到推荐 Product。详情桌面以大图和说明双栏建立作品身份；移动端收紧封面而不裁切作品，再以 2.4rem 完整标题、summary、三列事实和完整演示披露进入“制作准备”。材料、工具、成品与 Tutorial 继续在后续顺读区按 Shopify / Project 事实分流，不把 Product 价格或库存写回 Project 身份。
...
- **Don't** 将 Issue #23 两项 `resolved` / `ship` 扩大解释为英文发布、咨询发送、旧首页 build gate 或整体技术验收通过。
